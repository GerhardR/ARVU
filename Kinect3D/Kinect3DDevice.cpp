	...
	for(int y = 0; y < H; ++y)
		for( int x = 0; x < W; ++x){
			...
			if(colorX < 0 || colorX > 639 || colorY < 0 || colorY > 479)
				continue;
			const Vector4 pos = NuiTransformDepthImageToSkeletonF(x/float(W), y/float(H), isUsingSkeleton() ? d : d << 3);
...
void MyKinect::make3DSkeletonPoints( vector<Point> & background, vector<Point> & player1, vector<Point> & player2) const {
	...
	for(int y = 0; y < H; ++y)
		for( int x = 0; x < W; ++x){
			...
			if(colorX < 0 || colorX > 639 || colorY < 0 || colorY > 479)
				continue;
			const Vector4 pos = NuiTransformDepthImageToSkeletonF(x/float(W), y/float(H), d );
